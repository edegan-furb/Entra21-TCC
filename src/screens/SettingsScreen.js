...
function SettingsScreen() {
  ...
  const alertDeletAccountText =
    language === "en" ? "Delete Account" : "Deletar Conta";
  const alertDeletAccountSubText =
    language === "en"
      ? "Are you sure you want to delete your account? This action cannot be undone."
      : "Tem certeza que deseja excluir sua conta? Essa ação não pode ser desfeita.";
  async function deleteAccountHandler() {
    Alert.alert(alertDeletAccountText, alertDeletAccountSubText, [
      {
  ...
  return (
    ...
    >
      <View
        style={[
          styles.contentPerfil,
          { borderBottomWidth: 2, borderColor: colors.border500 },
        ]}
      >
        <InfPerfil />
      </View>
      ...
      <View style={styles.containerBody}>
        <View style={styles.contentBody}>
          <Text style={[styles.titleSettings, { color: colors.text900 }]}>
            Settings
          </Text>
          <SettingsItem
            nameIcon={"sunny-outline"}
            text={<TranslatedText enText="Theme" ptText="Tema" />}
            activeText={"🌙"}
            inActiveText={"☀"}
            swichTheme={true}
            swich={true}
          ...
          <SettingsItem
            ...
            text={<TranslatedText enText="Language" ptText="Idioma" />}
            activeText={"Br"}
            inActiveText={"En"}
            swichLanguage={true}
            swich={true}
          ...
          <SettingsItem
            ...
            text={<TranslatedText enText="About" ptText="Sobre" />}
            onPress={() => setModalVisible(true)}
          />
          <SettingsItem
            nameIcon={"close-circle-outline"}
            text={
              <TranslatedText enText="Delete account" ptText="Deletar conta" />
            }
            onPress={deleteAccountHandler}
          />
          <SettingsItem
            nameIcon={"log-out-outline"}
            text={<TranslatedText enText="Logout" ptText="Sair" />}
            onPress={authCtx.logout}
          />
        </View>
      </View>
      <Modal animationType="slide" transparent={true} visible={modalVisible}>
        <ModalInformationsPerfil onPress={() => setModalVisible(false)} />
      </Modal>
    </SafeAreaView>
  );
}
